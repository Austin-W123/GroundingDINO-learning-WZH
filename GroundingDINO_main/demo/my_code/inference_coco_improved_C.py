...
def run_inference(img_path, prompt, img_id, cls_ids):
   ...
   # 加载最优阈值配置（只需加载一次）
    if not hasattr(run_inference, "thresholds"):
        threshold_path = os.path.join(OUTPUT_DIR, 'best_thresholds.json')
        try:
            with open(threshold_path, 'r') as f:
                run_inference.thresholds = json.load(f)
            print(f"✅ 加载类别自适应阈值配置: {threshold_path}")
        except FileNotFoundError:
            run_inference.thresholds = {}
            print(f"⚠️ 未找到阈值配置文件 {threshold_path}，使用默认阈值0.25")
    ...
    try:
        ...
        for bbox, logit, phrase in zip(bboxes, logits, phrases):
            ...
            # 使用类别特定阈值
            default_thr = 0.25
            category_thr = float(run_inference.thresholds.get(str(cat_id), default_thr))
            
            if score < category_thr:
                continue
...
def main():
    ...
    # 保存
    seen_file = os.path.join(OUTPUT_DIR, f"coco_seen_400imgs_{PROMPT_ROUND}_improved_C.json")
    unseen_file = os.path.join(OUTPUT_DIR, f"coco_unseen_100imgs_{PROMPT_ROUND}_improved_C.json")
